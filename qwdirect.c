...
// Для ECC- R-S
if (! (is_chipset("MDM9x25") || is_chipset("MDM9x3x") || is_chipset("MDM9x4x") || is_chipset("MDM9x55"))) set_blocksize(516,1,10); // data - 516, spare - 1 байт, ecc - 10
// Для ECC - BCH
else {
//	set_udsize(516); // data - 516, spare - 2 или 4 байта
//	set_sparesize(sparnum);
	printf(" Just do nothing for NAND controller init ;-)\n");
}
}  
